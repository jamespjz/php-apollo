...
class ApolloClient
{
	...
	public $suffix_name; //生成文件后缀名
	public $session; //ini文件的session
    ...
    public function getConfigFile($namespaceName) {
        return $this->save_dir.DIRECTORY_SEPARATOR.$namespaceName.'.'.$this->suffix_name;
    }
    ...
    public function pullConfig($namespaceName) {
        ...
        if ($httpCode == 200) {
			...
			switch($this->suffix_name){
				case 'php':
					$content = '<?php '.PHP_EOL.'return ' . var_export($result['configurations'], true) . ';';
					file_put_contents($config_file, $content);
					break;
				case 'ini':
					$content .= !empty($this->session)?'['.$this->session.'] '.PHP_EOL:'';
					foreach($result['configurations'] as $key=>$value){
						$msg = $key.'="'.$value.'"';
						$content .= PHP_EOL.trim($msg, ',');
						file_put_contents($config_file, $content);
					}
					break;
				default:
					$content = '<?php return ' . PHP_EOL.var_export($result, true) . ';';
					file_put_contents($config_file, $content);
					break;
			}
        }elseif ($httpCode != 304) {
